 ...
 *
 * Verifies the user is a member of the organization specified in the input.
 * Adds organization and memberRole to the context.
 */
...
/**
 * Organization Editor procedure - requires EDITOR, ADMIN, or OWNER role
 */
export const orgEditorProcedure = orgProcedure.use(({ ctx, next }) => {
  assertRole(ctx.memberRole, "EDITOR");
  return next();
});
...
export const orgAdminProcedure = orgProcedure.use(({ ctx, next }) => {
  assertRole(ctx.memberRole, "ADMIN");
  return next();
});
...
export const orgOwnerProcedure = orgProcedure.use(({ ctx, next }) => {
  assertRole(ctx.memberRole, "OWNER");
  return next();
});
